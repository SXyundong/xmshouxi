...
type WorkflowPhase = 'idle' | 'previewing' | 'executing';
...
function StatusBadge({ phase, job, preview, result, error }: { phase: WorkflowPhase; job: LogisticsWorkflowJob | null; preview: LogisticsWorkflowPreview | null; result: LogisticsWorkflowResult | null; error: string }) {
...
function ToolSummary({ phase, job, preview, result, error }: { phase: WorkflowPhase; job: LogisticsWorkflowJob | null; preview: LogisticsWorkflowPreview | null; result: LogisticsWorkflowResult | null; error: string }) {
...
export default function WorkflowToolbar({ department, compact = false }: Props) {
  ...
  if (compact) {
    ...
    return <div className="relative z-30"><button type="button" onClick={() => setMobileOpen(true)} className="flex w-full items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.035] px-3 py-2 text-left"><span className="flex h-6 w-6 items-center justify-center rounded-lg bg-cyan-300/[0.08] text-cyan-100">↗</span><span className="min-w-0 flex-1"><span className="block text-[10px] font-medium text-white/70">部门工具</span><span className="mt-0.5 block truncate text-[9px] text-white/30">{preview ? `销量同步待确认 · ${preview.matched_rows} 行` : phase === 'previewing' ? `销量同步运行中 · ${job?.progress || 0}%` : result ? `销量同步已完成 · ${result.updated_rows} 行` : '销量同步 · 点击打开工具栏'}</span></span><span className="text-[10px] text-white/30">→</span></button>{mobileOpen && <><button type="button" aria-label="关闭工具栏" onClick={() => setMobileOpen(false)} className="fixed inset-0 z-40 bg-black/50 backdrop-blur-sm" /><div className="fixed inset-x-3 bottom-3 top-20 z-50 overflow-y-auto rounded-2xl border border-white/[0.1] bg-[#171a20]/95 p-3 shadow-2xl shadow-black/50 backdrop-blur-2xl"><div className="mb-3 flex items-center justify-between px-1"><div className="text-xs font-medium text-white/75">部门工具</div><button type="button" onClick={() => setMobileOpen(false)} className="rounded-lg px-2 py-1 text-sm text-white/35 hover:bg-white/[0.06] hover:text-white/70">×</button></div><ToolCard drawer /></div></>}</div>;
  }

  return <aside className="glass-strong relative hidden min-w-0 shrink-0 flex-col rounded-2xl xl:flex" style={{ width: panelWidth }}><div role="separator" aria-label="拖动调整工具栏宽度" onPointerDown={beginResize} onDoubleClick={resetWidth} className="group absolute -left-1.5 inset-y-0 z-10 flex w-3 cursor-col-resize items-center justify-center"><span className="h-10 w-0.5 rounded-full bg-white/10 transition group-hover:h-16 group-hover:bg-emerald-200/60" /></div><div className="flex items-center justify-between border-b border-white/[0.07] px-4 py-4"><div className="flex items-center gap-2 text-xs font-medium text-white/80"><svg className="h-4 w-4 text-emerald-200/70" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M12 3v4m0 10v4M3 12h4m10 0h4M5.6 5.6l2.8 2.8m7.2 7.2 2.8 2.8m0-12.8-2.8 2.8m-7.2 7.2-2.8 2.8" strokeLinecap="round" /></svg>部门工具</div><span className="rounded-full bg-white/[0.05] px-2 py-1 text-[9px] text-white/30">{busy ? '1 个运行中' : '1 个工具'}</span></div><div className="flex-1 overflow-y-auto p-3"><ToolCard /></div><div className="border-t border-white/[0.07] px-4 py-3 text-[9px] leading-4 text-white/20">可拖动左侧边缘调整宽度 · 双击恢复默认。网络路径写入已关闭。</div></aside>;
}
