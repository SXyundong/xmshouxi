...
// the form is immediately runnable with the scenario used to validate V0.9.
...
const solverStatusLabel:any = {
  PORTFOLIO_OPTIMAL:'候选模式最优',
  PROVEN_OPTIMAL:'已证明最优',
  BEST_FOUND:'当前最好方案',
  INFEASIBLE_PROVEN:'已证明无解',
};

function InstancedBoxes({placements,color,onSelect,dimmed}:{placements:any[];color:string;onSelect:(p:any)=>void;dimmed:boolean}){
  const ref=useRef<THREE.InstancedMesh>(null); const edgeRef=useRef<THREE.InstancedMesh>(null); const dummy=useMemo(()=>new THREE.Object3D(),[]); const edgeDummy=useMemo(()=>new THREE.Object3D(),[]);
  useEffect(()=>{if(!ref.current||!edgeRef.current)return; placements.forEach((p,index)=>{const visualSeparator=.002;const edgeSeparator=.004;const center=[(p.x+p.length/2)/1000,(p.z+p.height/2)/1000,(p.y+p.width/2)/1000] as [number,number,number];dummy.position.set(...center);dummy.scale.set(Math.max(.005,p.length/1000-visualSeparator),Math.max(.005,p.height/1000-visualSeparator),Math.max(.005,p.width/1000-visualSeparator));dummy.updateMatrix();ref.current!.setMatrixAt(index,dummy.matrix);edgeDummy.position.set(...center);edgeDummy.scale.set(Math.max(.005,p.length/1000+edgeSeparator),Math.max(.005,p.height/1000+edgeSeparator),Math.max(.005,p.width/1000+edgeSeparator));edgeDummy.updateMatrix();edgeRef.current!.setMatrixAt(index,edgeDummy.matrix)});ref.current.instanceMatrix.needsUpdate=true;edgeRef.current.instanceMatrix.needsUpdate=true},[placements,dummy,edgeDummy]);
  return <><instancedMesh ref={ref} args={[undefined,undefined,placements.length]} renderOrder={dimmed?0:2} onClick={(e:ThreeEvent<MouseEvent>)=>{e.stopPropagation();if(e.instanceId!==undefined)onSelect(placements[e.instanceId])}}><boxGeometry args={[1,1,1]}/>{dimmed?<meshBasicMaterial color="#6f8eaf" transparent opacity={.24} depthWrite={false} depthTest={false} wireframe/>:<meshStandardMaterial color={color} roughness={.52} metalness={.05} emissive={color} emissiveIntensity={.16}/>}</instancedMesh><instancedMesh ref={edgeRef} args={[undefined,undefined,placements.length]} renderOrder={dimmed?1:3} raycast={()=>null}><boxGeometry args={[1,1,1]}/><meshBasicMaterial color={dimmed?'#6b88a6':'#24496c'} transparent opacity={dimmed?.4:.72} depthWrite={false} depthTest={!dimmed} wireframe/></instancedMesh></>
}
function CameraPreset({view,controls}:{view:string;controls:React.RefObject<any>}){const{camera}=useThree();useEffect(()=>{const positions:Record<string,[number,number,number]>={perspective:[11.5,5.8,7],top:[6,15,1.15],side:[6,2.8,12],door:[15,2.5,1.15],head:[-3,2.5,1.15]};camera.position.set(...positions[view]);camera.up.set(0,1,0);controls.current?.target.set(6,1.3,1.15);controls.current?.update()},[view,camera,controls]);return null}
function Scene({solution,items,view,onSelect,focusSku,showContext}:{solution:any;items:SkuRow[];view:string;onSelect:(p:any)=>void;focusSku:string|null;showContext:boolean}){const controls=useRef<any>(null);return <Canvas camera={{position:[11.5,5.8,7],fov:50}} onPointerMissed={()=>onSelect(null)}><color attach="background" args={['#09111f']}/><ambientLight intensity={1.35}/><directionalLight position={[8,12,5]} intensity={2}/><gridHelper args={[16,32,'#314158','#17243a']} position={[6,0,1.18]}/><mesh position={[6.016,1.345,1.176]} raycast={()=>null}><boxGeometry args={[12.032,2.69,2.352]}/><meshBasicMaterial transparent opacity={0} depthWrite={false}/><Edges color="#9db0ca"/></mesh>{items.map(item=>{const p=solution?.placements?.filter((x:any)=>x.sku===item.solverSku)||[];const dimmed=focusSku!==null&&focusSku!==item.solverSku;if(!p.length||dimmed&&!showContext)return null;return <InstancedBoxes key={item.productId} placements={p} color={item.color} dimmed={dimmed} onSelect={onSelect}/>})}<OrbitControls ref={controls} target={[6,1.3,1.15]} minDistance={5.5} maxDistance={24} enableDamping dampingFactor={.08}/><CameraPreset view={view} controls={controls}/></Canvas>}

export default function ContainerLoadingOptimizerPanel(){

  const [items,setItems]=useState(initialItems); const [activeMode,setActiveMode]=useState('UNIFIED_STAGE_MAX');
  ...
  const autoItem=items[items.length-1]; const autoQuantity=autoItem&&solution?.sku_quantities?.[autoItem.solverSku];
  function edit(index:number,key:keyof SkuRow,value:string){
    setResults({});setSelected(null);setFocusSku(null);
    setItems(rows=>rows.map((row,i)=>i===index?{...row,[key]:key==='sku'||key==='color'||key==='productId'||key==='solverSku'||key==='msku'||key==='productName'||key==='country'||key==='store'?value:key==='max'&&value.trim()===''?null:Number(value)}:row));
  }
  function editFixedQuantity(index:number,value:string){
    const quantity=Math.max(0,Number(value)||0);
    setResults({});setSelected(null);setFocusSku(null);
    setItems(rows=>rows.map((row,i)=>i===index?{...row,min:quantity,max:quantity}:row));
  }
  ...
  function addProduct(candidate:ProductCandidate){
    ...
    setItems(rows=>{const previous=rows.map((row,index)=>index===rows.length-1?{...row,min:Math.max(1,row.min),max:Math.max(1,row.min)}:row);return [...previous,{productId:candidate.id,solverSku:`PRODUCT::${candidate.id}`,sku:candidate.sku,msku:candidate.msku,productName:candidate.product_name,country:candidate.country,store:candidate.store,l:dimensions[0] as number,w:dimensions[1] as number,h:dimensions[2] as number,weight:dimensions[3] as number,min:0,max:null,step:1,stage:Math.max(1,...rows.map(row=>row.stage)),color:`hsl(${(next*83)%360}, 75%, 62%)`}]});setCandidates([]);setIdentifier('');
  }
  function removeSku(index:number){setResults({});setSelected(null);setFocusSku(null);setItems(rows=>{const remaining=rows.filter((_,i)=>i!==index);return remaining.map((row,i)=>i===remaining.length-1?{...row,min:0,max:null}:row)})}
  ...
  return <div className="container-loading-tool"><div className="app-shell">
    <header><div><span className="eyebrow">40HQ · MULTI-SKU 3D OPTIMIZER</span><h1>智能三维装柜优化系统</h1></div><div className="header-status"><span className="live-dot"/>V0.9 · Replayable Carton Loading</div></header>
    <div className="workspace">
      ...
      <main className="main-stage">
        <div className="metric-row">
          ...
          <div className="metric-card gap"><span>AUTO FILL</span><strong>{autoQuantity??'—'}</strong><small>boxes</small></div>
          <div className={`metric-card status ${valid?'ok':''}`}><span>VALIDATION</span><strong>{solution?(valid?'PASS':'FAIL'):'—'}</strong><small>{solution?(solverStatusLabel[solution.solution_status]||solution.solution_status):'等待求解'}</small></div>
        </div>
        <section className="viewer-card">
          ...
          <div className="canvas-wrap"><Scene solution={solution} items={items} view={view} focusSku={focusSku} showContext={showContext} onSelect={(placement:any)=>{setSelected(placement);if(placement){setFocusSku(placement.sku);setShowContext(false)}}}/><div className="door-label">DOOR →</div>{!solution&&!busy&&<div className="empty-state"><b>准备进行真实三维混装</b><span>设置固定数量和装载阶段；最后一项自动填充，然后计算装柜方案</span></div>}{busy&&<div className="solver-progress"><b>{jobMessage||'正在计算装柜方案…'}</b><span>进度 {jobProgress}%</span></div>}<div className="color-legend">{items.map(i=><button className={focusSku===i.solverSku?'active':''} key={i.productId} onClick={()=>{const nextFocus=focusSku===i.solverSku?null:i.solverSku;setFocusSku(nextFocus);setShowContext(false)}}><i style={{background:i.color}}/>{i.sku}</button>)}{focusSku&&<button className="show-all" onClick={()=>{setFocusSku(null);setShowContext(false)}}>显示全部</button>}</div>{focusSku&&<div className="focus-badge"><span>聚焦 <b>{items.find(item=>item.solverSku===focusSku)?.sku||focusSku}</b></span><button className="focus-context-toggle" onClick={()=>setShowContext(value=>!value)}>{showContext?'隐藏其它商品':'显示其它商品'}</button><button aria-label="取消聚焦" onClick={()=>{setFocusSku(null);setShowContext(false)}}>×</button></div>}{selected&&<div className="selection-pop"><button onClick={()=>setSelected(null)}>×</button><b>{selected.box_id}</b><span>{items.find(item=>item.solverSku===selected.sku)?.sku||selected.sku} · Stage {selected.loading_stage} · O{selected.orientation}</span><span>XYZ {selected.x}, {selected.y}, {selected.z} mm</span><span>{selected.length} × {selected.width} × {selected.height} mm</span></div>}</div>
        </section>
        <section className="data-card"><div className="tabs"><button className={panel==='sku'?'active':''} onClick={()=>setPanel('sku')}>SKU 数量</button><button className={panel==='blocks'?'active':''} onClick={()=>setPanel('blocks')}>Block 列表</button><button className={panel==='sequence'?'active':''} onClick={()=>setPanel('sequence')}>装载顺序</button><button className={panel==='validation'?'active':''} onClick={()=>setPanel('validation')}>验证报告</button><div className="search-delta">{solution?.auto_fill_upper_quantity!=null?<>AUTO 理论上界 {solution.auto_fill_upper_quantity} 箱 <b>差 {solution.auto_fill_gap_boxes??'—'} 箱</b><em>· 未计入三维摆放</em></>:<>等待数量上界</>}</div></div>
          <div className="table-wrap">{panel==='sku'&&<table><thead><tr><th>SKU</th><th>箱规 (cm)</th><th>数量规则</th><th>阶段</th><th>最终箱数</th><th>货物体积</th></tr></thead><tbody>{items.map((i,index)=>{const q=solution?.sku_quantities?.[i.solverSku]||0;const isAuto=index===items.length-1;return <tr className={focusSku===i.solverSku?'focus-row':''} key={i.productId} onClick={()=>setFocusSku(focusSku===i.solverSku?null:i.solverSku)}><td><i className="sku-dot" style={{background:i.color}}/>{i.sku}{i.msku&&<small className="table-subtitle">{i.msku}</small>}</td><td>{i.l} × {i.w} × {i.h}</td><td>{isAuto?'自动填充（不限）':`固定 ${i.min}`}</td><td>{i.stage}</td><td><b>{solution?q:'—'}</b></td><td>{solution?(q*i.l*i.w*i.h/1e6).toFixed(3):'—'} m³</td></tr>})}</tbody></table>}
          ...
          {panel==='validation'&&<div className="validation-grid">{[['no_overlap','无碰撞'],['within_container','全部在柜内'],['supported','100%最终支撑'],['quantity_constraints','Min / Max'],['legal_orientations','合法方向'],['door_valid','门宽/门高'],['accessibility_valid','柜门直线可达'],['factory_sequence_valid','阶段与 SKU 顺序'],['stack_limit_valid','堆叠层数'],['top_load_valid','顶部承重'],['fragile_valid','易碎约束']].map(v=><div key={v[0]} className={solution?.validation?.[v[0]]?'pass':''}><span>{solution?(solution.validation[v[0]]?'✓':'×'):'—'}</span><b>{v[1]}</b></div>)}</div>}</div>
        </section>
      </main>
    </div>
  </div></div>
