...
function Scene({solution,items,view,onSelect,focusSku}:{solution:any;items:SkuRow[];view:string;onSelect:(p:any)=>void;focusSku:string|null}){const controls=useRef<any>(null);return <Canvas camera={{position:[11.5,5.8,7],fov:50}} onPointerMissed={()=>onSelect(null)}><color attach="background" args={['#09111f']}/><ambientLight intensity={1.35}/><directionalLight position={[8,12,5]} intensity={2}/><gridHelper args={[16,32,'#314158','#17243a']} position={[6,0,1.18]}/><mesh position={[6.016,1.345,1.176]} raycast={()=>null}><boxGeometry args={[12.032,2.69,2.352]}/><meshBasicMaterial transparent opacity={0} depthWrite={false}/><Edges color="#9db0ca"/></mesh>{items.map(item=>{const p=solution?.placements?.filter((x:any)=>x.sku===item.solverSku)||[];return p.length>0&&<InstancedBoxes key={item.productId} placements={p} color={item.color} dimmed={focusSku!==null&&focusSku!==item.solverSku} onSelect={onSelect}/>})}<OrbitControls ref={controls} target={[6,1.3,1.15]} minDistance={5.5} maxDistance={24} enableDamping dampingFactor={.08}/><CameraPreset view={view} controls={controls}/></Canvas>}
